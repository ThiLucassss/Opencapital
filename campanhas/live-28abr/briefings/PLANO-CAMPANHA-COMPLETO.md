**UMA APLICAÇÃO**
**OPEN**CAPITAL

`CAMPANHA — LIVE 28/04 ÀS 20H · V2 EXECUTADA`

# Convite Matador
# Live IA · Print & Trade

5 emails + 5 WhatsApps + 1 mensagem extra de calendário · disparo em **grupos** de WhatsApp · captação por **1 clique** (sem formulário) · base ~5.004 leads no AC. Vídeos do YouTube como prova viva.

`OPERACIONAL V2 EXECUTADA · ABR/2026`  ·  `11 PEÇAS · 6 DIAS`

---

## 01  Visão geral e cronograma

> Decisões mantidas do brief: sem inscrição (1 clique = lista + tag), vídeos do YouTube como prova viva, agressividade máxima, foco em "como a IA funciona". Decisões refinadas no chat: WhatsApp em **grupos** (não 1-a-1), 2 links sempre, 2 CTAs no email, valores informais anti-spam, mensagem extra de calendário (W2.5).

### Os 4 vídeos (LINK 1-4)

| # | Vídeo | YouTube ID | Views |
|---|---|---|---|
| LINK 1 | 1.700 reais de lucro com IA para day trade | `yeUXsAWhHSw` | 5,2 mil |
| LINK 2 | FAZENDO DAY TRADE COM IA — PASSO A PASSO | `2HV4noYpbaE` | 9,2 mil |
| LINK 3 | GANHEI R$ 2.255,00 DE LUCRO COM ESSA IA | `gG2bUUwDFpE` | 3,6 mil |
| LINK 4 | A MELHOR IA PARA GANHAR COM DAY TRADE — Inteligência Artificial | `_wwMqYp2ze0` | 4,6 mil |

Total: 22,6 mil views em 8 dias.

### Cronograma das 11 peças

| Dia | Email | WhatsApp grupo | Vídeos | Status |
|---|---|---|---|---|
| **SEX 24/04** · 17h | E1 — Anúncio + ataque ao "estudou anos" | W1 — Aviso curto (17h30) | 1 | ✅ Enviado |
| **SÁB 25/04** · 17h | E2 — Promessa falsa morta | W2 — Provocação (17h30) | 3 | ✅ Enviado |
| **DOM 26/04** | — | **W2.5 — Add na agenda** (extra) | — | 🟡 Hoje |
| **SEG 27/04** · 19h | E3 — Recap + último call | W3 — Áudio Suriel 50s (20h) | 4 | 🟢 A fazer |
| **TER 28/04** · 9h | E4 — É hoje | W4 — Lembrete (12h) | — | 🟢 A fazer |
| **TER 28/04** · 19h | E5 — 1h antes (link da live) | W5 — 5 min antes (19h55) | — | 🟢 A fazer |
| **TER 28/04** · **20h00** | — | — | — | 🔴 LIVE |

---

## 02  Setup técnico e métricas

### ActiveCampaign (já configurado)

- Lista: `LIVE-28ABR-CONFIRMADOS` (ID 8)
- Tag: `quer-live-ia-28abr` (ID 9)
- Automation: Site Tracking detecta `/live-28abr-confirmado` → adiciona à lista + aplica tag

### URLs e UTMs

| Onde | URL | UTM padrão |
|---|---|---|
| CTA email | `https://lp.theopencapital.org/live-28abr-confirmado` | `?utm_source=email&utm_medium=campaign&utm_campaign=live28abr&utm_content=email{N}-cta` |
| CTA WhatsApp | `https://lp.theopencapital.org/convite-live-28abr` | (sem UTM no incoming, captura por Site Tracking) |
| Vídeos email | `https://www.youtube.com/watch?v=<id>&utm_source=email&...&utm_content=email{N}-video1` | — |

### Métricas-alvo

| Métrica | Mín | Saudável | Excelente |
|---|---|---|---|
| Abertura email | 30% | 40% | 50%+ |
| Clique CTA email | 15% (~735) | 25% (~1.225) | 35%+ (~1.715) |
| Clique vídeo YouTube | 5% (~245) | 10% (~490) | 15%+ (~735) |
| Confirmados → ao vivo | 40% | 55% | 70%+ |
| Vendas via live | 20 | 60 | 120+ |

### Pendências bloqueadoras

1. **Áudio do W3** — Suriel grava 50-60s até dom 27/04 18h
2. **Automation no AC** — clique CTA email → tag + lista (ainda não criada)

> A oferta exclusiva é decisão do Suriel, anunciada ao vivo na transmissão — não entra no documento.

---

# SEÇÃO 1 DE 2

# EMAILS

5 mensagens · 6 dias · agressividade máxima
Cada uma usa um gatilho psicológico diferente. Cada uma linka pra prova viva no YouTube.

---

### EMAIL 1 — ANÚNCIO + ATAQUE  `Sex 24/04 · 17h · D-4` ✅

**▼ Gatilho:** ATAQUE AO "ESTUDOU ANOS SEM RESULTADO"

| | |
|---|---|
| **ASSUNTO** | Você ainda tá estudando análise técnica em 2026? |
| **PREVIEW** | 22 mil views em 8 dias mostram o que a IA tá fazendo. Live terça 20h. |
| **VÍDEOS** | LINK 1 + LINK 2 |

> %FIRSTNAME%, pergunta direta — quanto tempo você gastou nos últimos 3 anos olhando vídeo de YouTube? 100h? 500h? 1.000h? Soma. Provavelmente passou de cinco mil. Resultado? Continuo perdendo. Não tô te julgando — fui isso por 8 anos. [CTA TOP]. Mas presta atenção: [VÍDEO 1 + VÍDEO 2]. 22 mil views em 8 dias. Estudar análise técnica em 2026 é como aprender cálculo no ábaco. [HERO DATE 28/04 20H00]. Terça vou rodar a IA ao vivo em 4 ativos + oferta exclusiva. 1 clique. [CTA BOTTOM]. P.S. — Se você abriu mas não vai clicar, daqui a 6 meses você vai estar onde tá hoje.

**🔧 Adaptações aplicadas vs brief:**
- Valores em R$ informais ("cinco mil")
- **2 botões CTA** (top above-the-fold + bottom) — brief tinha 1 só
- Header tag vermelho com countdown (`AO VIVO · TER 28/04 · 20H`)
- Hero date block preto com 28/04 · 20H00 destacado
- Sub-CTA "TER 28/04 · 19H → link no WhatsApp"
- Assinatura caligráfica + "Embaixador OpenCapital" (decisão de manter)
- Footer legal Ports Capital + unsubscribe
- Sem imagens externas além das thumbnails YouTube

**Arquivo:** `emails/E1-anuncio-ataque.html`

---

### EMAIL 2 — PROMESSA FALSA MORTA  `Sáb 25/04 · 17h · D-3` ✅

**▼ Gatilho:** ATAQUE À INDÚSTRIA DE GURUS

| | |
|---|---|
| **ASSUNTO** | A era das promessas falsas acabou (terça 20h) |
| **PREVIEW** | 2.255 reais de lucro em uma operação. Sem curso, sem grupo, sem guru. |
| **VÍDEO** | LINK 3 |

> %FIRSTNAME%, vou te falar uma coisa que ninguém no mercado fala: **A era das promessas falsas acabou.** Aquele guru com Lamborghini alugada que te vendeu mentoria de dez mil. Aquele grupo VIP de Telegram que sumiu. Aquele robô de copy trade que zerou a banca. Aquele curso de quase 4 mil que te ensinou Fibonacci mas não te ensinou apertar botão sem tremer. **Tudo isso morreu na segunda metade de 2025.** A tecnologia chegou no celular do trader. [CTA TOP]. Olha: [VÍDEO 3 — R$ 2.255]. Não é mágica. É o resultado normal de quem opera com tecnologia institucional. [HERO DATE]. Terça 28/04 · 20h00 ao vivo. [CTA BOTTOM]. P.S. — Eu não vendo curso. **Eu vendo a saída.**

**🔧 Adaptações aplicadas vs brief:**
- Brief original: data sex 25/04 11h · executado: sáb 25/04 17h (atraso reposicionado)
- Valores informais ("dez mil", "quase 4 mil")
- 2 botões CTA (top + bottom)
- Mesmo padrão visual do E1 (header tag, hero date, sub-CTA, assinatura, footer)
- P.S. mantido literal do brief ("Eu vendo a saída")

**Arquivo:** `emails/E2-promessa-falsa.html`

---

### EMAIL 3 — RECAP + ÚLTIMO CALL  `Seg 27/04 · 19h · D-1` 🟢

**▼ Gatilho:** ÚLTIMA CHANCE + DOR ESPECÍFICA

| | |
|---|---|
| **ASSUNTO** | Amanhã 20h · sem mão tremendo, sem dúvida, sem estudo eterno |
| **PREVIEW** | A IA resolve em 2 segundos o que você não consegue há 5 anos. |
| **VÍDEO** | LINK 4 |

> %FIRSTNAME%, amanhã 20h00 ao vivo. Antes de te falar do que vou mostrar: você abre o gráfico, vê uma entrada que parece boa, **a mão treme**. "Será que é mesmo? Deixa eu olhar mais um indicador..." 5 minutos depois o preço já foi. Esse padrão tem quanto tempo? 5 anos? 8 anos? Ou o oposto: aperta na primeira coisa pra recuperar a perda, dá errado, segura achando que volta, sai no stop perdendo o dobro. **Isso vai parar quando?** [CTA TOP]. Acabou de parar pra todo mundo que assistiu: [VÍDEO 4]. 4.600 pessoas em 8 dias. **A saída é uma IA que faz a análise por você. Você só executa.** [HERO DATE — AMANHÃ]. 90 min · 4 ativos · oferta exclusiva no fim. [CTA BOTTOM]. P.S. — Continuar fazendo o que tá fazendo (e ter o mesmo resultado). Ou clicar agora. **Pensa direito.**

**🔧 Adaptações aplicadas vs brief:**
- Header tag muda pra `FALTA 1 DIA · LIVE TER 28/04 · 20H`
- Hero date block adapta texto pra "AMANHÃ · TERÇA 28 DE ABRIL · 20H00"
- 2 botões CTA (top + bottom)
- Assunto encurtado ("mão tremendo" no lugar de "decisão emocional" — mais visceral)
- Resto do padrão visual mantido

**Arquivo:** `emails/E3-recap-ultimo-call.html` (a criar)

---

### EMAIL 4 — É HOJE  `Ter 28/04 · 9h · DIA D` 🟢

**▼ Gatilho:** COMPROMISSO + EXPECTATIVA

| | |
|---|---|
| **ASSUNTO** | É HOJE · 20h · você vai ver a IA operando |
| **PREVIEW** | Bloqueia 90 minutos. Sai do trabalho 19h45 se for o caso. Não tem replay completo. |
| **VÍDEO** | — |

> %FIRSTNAME%, é hoje. 20h00 ao vivo, sem replay. 3 coisas pra tirar máximo dessa noite: **(1) Bloqueia 90 min sem distração** — tira da agenda, pede pra família. **(2) Pensa antes na sua maior dor com trading** — anota, no Q&A você faz a pergunta certa. **(3) Reserva um momento pra decidir, no fim** — oferta exclusiva nos últimos 15 min, quem chega depois não pega. [CTA TOP]. Link da transmissão pelo WhatsApp 19h. Salva o número da OpenCapital pra não cair em spam. [HERO DATE — HOJE 28/04 20H00]. [CTA BOTTOM]. (Se ainda não confirmou, clica agora — vou liberar acesso só pra quem confirmou.) P.S. — Oferta tem condições específicas que não posso replicar depois. Se não estiver lá entre 21h15-21h30, perde. **É ao vivo ou nada.**

**🔧 Adaptações aplicadas vs brief:**
- Header tag pulsa: `● HOJE · 20H · AO VIVO`
- Hero date block muda pra "HOJE · 28 DE ABRIL · 20H00 EM PONTO"
- 2 botões CTA (top + bottom)
- Sub-CTA muda pra "LINK DA TRANSMISSÃO → no WhatsApp às 19h"
- Resto do padrão mantido

**Arquivo:** `emails/E4-e-hoje.html` (a criar)

---

### EMAIL 5 — 1 HORA ANTES  `Ter 28/04 · 19h · 1H ANTES` 🟢

**▼ Gatilho:** AÇÃO IMEDIATA

| | |
|---|---|
| **ASSUNTO** | ⏰ 1 hora · link aqui dentro |
| **PREVIEW** | Clica agora pra confirmar conexão. Salva o link. Te vejo às 20h. |
| **VÍDEO** | — |

> %FIRSTNAME%, 1 hora. Link da transmissão: **[BOTÃO GRANDE — ENTRAR NA LIVE AGORA → {{LINK_DA_LIVE}}]**. Recomendações: ✓ clica agora e deixa aba aberta · ✓ celular na tomada · ✓ fecha abas pesadas · ✓ confere áudio 5 min antes · ✓ tira do silencioso. Começo 20h00. Espero 2 min e parto. **{{N_CONFIRMADOS}} pessoas confirmadas**. Capacidade do servidor: 2.000 simultâneos — se passar, alguém fica de fora por ordem de entrada. Se ainda não confirmou: [CTA]. Te vejo em 60 min. P.S. — Oferta começa 21h15. Se quer só "dar uma olhada e sair", entra mesmo assim. **Mas se quiser pegar a oferta, fica até o fim.**

**🔧 Adaptações aplicadas vs brief:**
- Header tag: `● COMEÇA EM 1 HORA · HOJE 20H`
- Botão de destaque (não os 2 padrões) — esse email é sobre o link da live, não conversão
- Placeholders `{{LINK_DA_LIVE}}` e `{{N_CONFIRMADOS}}` pra preencher na hora
- Bloco de checklist visual (✓) substituindo a lista corrida do brief
- 1 CTA fallback no fim pra quem ainda não confirmou

**Arquivo:** `emails/E5-1h-antes.html` (a criar)

---

# SEÇÃO 2 DE 2

# WHATSAPP GRUPOS

5 mensagens + 1 extra · 6 dias
**Disparo em grupos** (broadcast), não 1-a-1. Sem `[Nome]`. Sempre 2 links: 🎥 YouTube + 🔐 Landing.

---

### WHATSAPP 1 — AVISO CURTO  `Sex 24/04 · ~17h30` ✅

**▼ Gatilho:** ATAQUE CURTO + AVISO

> Pergunta rápida: você ainda tá *estudando análise técnica* em 2026? Esse vídeo é de semana passada — *1.700 reais de lucro em 1 operação*. 🎥 youtu.be/yeUXsAWhHSw . Terça *28/04 às 20h* eu rodo essa mesma IA ao vivo em 4 ativos. *Oferta exclusiva* só durante a transmissão. Sem inscrição. Sem replay. 🔐 lp.theopencapital.org/convite-live-28abr

**🔧 Adaptações aplicadas vs brief:**
- Adaptado pra **grupos** (sem `[Nome]`, sem "Suriel.")
- **2 links** (YouTube + landing) — brief só tinha 1 link de cada
- Tom direto, formatação `*negrito*` nativa do WhatsApp

**Arquivo:** `whatsapp/W1-aviso-curto.txt`

---

### WHATSAPP 2 — PROVOCAÇÃO  `Sáb 25/04 · ~17h30` ✅

**▼ Gatilho:** PROVOCAÇÃO + GURU LAMBORGHINI

> Quanto você já gastou em curso, grupo VIP, robô de copy trade? E o resultado? *A era das promessas falsas acabou.* 🎥 *2.255 de lucro com a IA em 1 operação:* youtu.be/gG2bUUwDFpE . Terça *28/04 às 20h* eu rodo essa IA ao vivo. Sem replay. Oferta exclusiva só pra quem tá presente. 🔐 *Confirmar:* lp.theopencapital.org/convite-live-28abr

**🔧 Adaptações aplicadas vs brief:**
- Brief: sex 25/04 14h · executado: sáb 25/04 17h30
- **Encurtado ~40%** (brief tinha "Soma. R$ 5 mil? R$ 10 mil? R$ 20 mil?" — cortei a soma porque o gancho já entrega)
- Sem `[Nome]` (grupo)
- 2 links

**Arquivo:** `whatsapp/W2-provocacao.txt`

---

### WHATSAPP 2.5 — ADD NA AGENDA  `Dom 26/04 · qualquer hora` 🟡

**▼ Gatilho:** EXTRA · COMPROMISSO DE CALENDÁRIO

> Coloca na agenda agora pra não esquecer 👇 *LIVE OpenCapital — Terça 28/04 às 20h* 🤖 IA ao vivo em 4 ativos · 🔐 Oferta exclusiva só ao vivo. Adiciona em 1 toque (Google Calendar): {{REBRANDLY_LINK}} . Se for iPhone e não abrir, o botão Apple Calendar tá aqui: lp.theopencapital.org/convite-live-28abr

**🔧 Adaptações aplicadas vs brief:**
- **Peça nova** (não estava no brief) — preenche o gap dom 26/04 entre W2 e W3
- Link Google Calendar gerado a partir do componente `Live28_CalButtons.tsx` da landing → encurtar via Rebrandly antes de mandar
- Fallback pra Apple via landing (que tem botões Google + Apple via .ics)
- Não conta como W3, **não substitui** o áudio do Suriel

**Arquivo:** `whatsapp/W2.5-add-agenda.txt` (a criar)

URL Google Calendar (encurtar no Rebrandly antes de enviar):
```
https://calendar.google.com/calendar/event?action=TEMPLATE&text=OpenCapital+-+Live+IA+Print+%26+Trade&dates=20260428T230000Z%2F20260429T003000Z&details=Live+ao+vivo...&location=100%25+Online
```

---

### WHATSAPP 3 — ÁUDIO SURIEL  `Seg 27/04 · 20h` 🟢

**▼ Gatilho:** ÁUDIO MATADOR + COMPROMISSO PESSOAL

**Formato:** áudio gravado no celular do Suriel (~50s) + 1 mensagem texto curta com link logo depois.

**Script para Suriel gravar** (tom: baixo, conversacional, NÃO palestra):

> "Oi, Suriel aqui. Tô gravando aqui na segunda à noite, um dia antes da live de amanhã. Olha, vou ser direto. Amanhã 20h é a live. Vou mostrar a IA funcionando ao vivo, vou abrir oferta exclusiva no fim, e não vai ter replay completo. Mas eu queria te falar uma coisa antes. Se você tá nesse grupo, é porque em algum momento de abril você se interessou pelo Print & Trade. Mas não foi pra frente. E eu sei o que tá rolando na sua cabeça desde lá. 'Será que funciona mesmo?'. 'Vou esperar pra ver mais resultado'. 'Próxima'. Cara, o resultado já tá no YouTube. 22 mil views em 8 dias. 1.700 num vídeo, 2.255 em outro, mais 6 mil em outro. Não é teoria. Não é promessa. Tá lá pra qualquer um ver. Amanhã eu vou mostrar AO VIVO como aquilo é feito. Por dentro. Em tempo real. Bota na agenda. 20h. Te mando o link uma hora antes."

**Texto pós-áudio (1 min depois):**

> 🔐 *Confirmar presença na live:* lp.theopencapital.org/convite-live-28abr

**🔧 Adaptações aplicadas vs brief:**
- Brief original mencionava "[Nome]" no áudio · adaptado pra **grupo** ("Se você tá nesse grupo")
- Adicionei menção ao **6 mil de lucro** (vídeo do canal não citado no brief, dá mais força à prova social)
- Texto pós-áudio em mensagem separada (forma mais natural no grupo)

**Arquivo:** `whatsapp/W3-audio-suriel.txt` (a criar com script + instruções)

---

### WHATSAPP 4 — LEMBRETE DIA D  `Ter 28/04 · 12h` 🟢

**▼ Gatilho:** LEMBRETE DIA D

> *É HOJE* 🔴 . 20h00 ao vivo. · Como a IA funciona por dentro · 4 análises em ativos diferentes · Q&A no chat · *Oferta exclusiva no fim* (só ao vivo). Te mando o link às 19h. Reserva o horário. Sai do trabalho 19h45 se for o caso. Vale a pena. 🔐 *Se ainda não confirmou:* lp.theopencapital.org/convite-live-28abr

**🔧 Adaptações aplicadas vs brief:**
- Adicionei link da landing pra quem ainda não confirmou (brief não tinha CTA aqui)
- 🔴 no lugar do emoji do brief (mais agressivo)

**Arquivo:** `whatsapp/W4-lembrete.txt` (a criar)

---

### WHATSAPP 5 — CHAMADA FINAL  `Ter 28/04 · 19h55` 🟢

**▼ Gatilho:** CHAMADA FINAL · 5 MIN

> *5 MINUTOS* 🔴 . → {{LINK_DA_LIVE}} . Tô entrando agora. Começo às 20h em ponto. Clica e entra antes que enche.

**🔧 Adaptações aplicadas vs brief:**
- Praticamente literal do brief (já estava enxuto)
- Placeholder `{{LINK_DA_LIVE}}` pra preencher na hora

**Arquivo:** `whatsapp/W5-5min.txt` (a criar)

---

# 03  Páginas Framer (já existentes)

### `/convite-live-28abr` — destino do WhatsApp

Hero badge `● AO VIVO · TER 28/04 · 20H` · H1 *"Você ainda tá estudando análise técnica em 2026?"* · vídeos LINK 1 + LINK 2 inline (iframe) · countdown · event card · CTA → `/live-28abr-confirmado` · reminder footer.

### `/live-28abr-confirmado` — thank you do email + CTA

Hero `✓ VOCÊ TÁ DENTRO` · H1 *"Te vejo terça às 20h"* · countdown · event card · botões Google Calendar + Apple Calendar (.ics) · reminder footer. **Site Tracking ativo** dispara a automation no AC.

---

# 04  Notas operacionais finais

### Pendências bloqueadoras (recap)

- ☐ **Áudio do W3** — Suriel grava até dom 27/04 18h
- ☐ **Automation no AC** — clique CTA email → tag + lista (ainda não criada)

### Substituições de última hora

- ☐ E5 e W5: substituir `{{LINK_DA_LIVE}}` pelo URL real da transmissão
- ☐ E5: substituir `{{N_CONFIRMADOS}}` pelo número real
- ☐ W2.5: substituir `{{REBRANDLY_LINK}}` pelo link encurtado

### Tarefas

**Pontuais:**
- ☐ Mandar W2.5 hoje 26/04 nos grupos
- ☐ Suriel gravar áudio W3 até dom 18h
- ☐ Criar automation no AC
- ☐ Disparar E3, W3, E4, W4, E5, W5 nos horários
- ☐ Suriel rodar a live terça 20h

**Recorrentes (durante a campanha):**
- ☐ Conferir aberturas/cliques no AC após cada disparo
- ☐ Conferir cliques nos vídeos no YouTube Studio (fonte "External")
- ☐ Conferir contagem da lista LIVE-28ABR-CONFIRMADOS subindo
